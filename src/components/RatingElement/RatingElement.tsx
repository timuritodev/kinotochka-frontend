...
const RatingElement: FC<IRating> = ({ id, rate }) => {
	...
	const handleRatingClick = async (value: number) => {
		setRating(value);
		if (rating === 0) {
			await dispatch(setRatingApi({ id, rate: value, token: user.token }));
		}
		else {
			await dispatch(updateRatingApi({ id, rate: value, token: user.token }));
		}
		...
		<>
			{user.token ? (
				<div className="rating-element">
					<h2 className="rating-element__text">Ваша оценка - {rate}</h2>
