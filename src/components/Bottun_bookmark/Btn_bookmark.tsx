...
import { updateFavorite, updateWatch } from 'src/services/redux/slices/films/films';

export const BtnBookmark = ({ nameTypes, id }: { nameTypes: string, id: string }) => {
    ...
    const typesImg = nameTypes === 'favorite'
        ? (filmFav ? img_bookmark_pressed : img_bookmark_default)
        : (filmWatch ? img_watch_pressed : img_watch_default)

    return (
        <section
            className='bookmark_favorite'
            onClick={nameTypes === 'favorite'
                ? handleClickFavorite
                : handleClickWatch}>
